...
app.post('/itemById', async (req, res) => {
  ...
  connection.getConnection(function (err, connection) {
    // Executing the MySQL query (select all data from the 'users' table).
    const itemsQuery = 'SELECT i.type, i.name, i.description, i.image, i.dateOfPurchase, i.storageSite, u.title, u.firstName, u.lastName, (SELECT GROUP_CONCAT(damageDescription) FROM damages WHERE item_ID = 1) AS damages FROM Items i INNER JOIN users u on i.user_short = u.short WHERE i.id = "' + req.body.id + '"'
